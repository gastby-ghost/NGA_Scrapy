...
class PlaywrightMiddleware:
    ...
    def process_request(self, request, spider):
        ...
        if time.time() - self.last_stat_time > 300:
            ...
            # 输出封禁检测报告
            if self.instance_manager:
                status_report = self.instance_manager.get_status_report()
                self.logger.info(status_report)

                # 立即检查实例健康状态（如果管理器存在）
                try:
                    from .utils.instance_manager import BrowserInstanceManager
                    if isinstance(self.instance_manager, BrowserInstanceManager):
                        self.instance_manager._check_instances_health()
                except Exception as e:
